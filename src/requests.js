...
export const getBasicHeroInfoById = async (id) => {
  const { data: powerstats } = await axios.get(
    `https://superheroapi.com/api/${ACCES_TOKEN}/${id}/powerstats`
  );
  const { data: image } = await axios.get(
    `https://superheroapi.com/api/${ACCES_TOKEN}/${id}/image`
  );
...
export const getDetailedInfoById = (id) => {
  return axios.get(`https://superheroapi.com/api/${ACCES_TOKEN}/${id}`);
};
